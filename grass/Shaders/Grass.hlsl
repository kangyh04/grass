...
void ApplyParent(int boneId)
{
    ...
    bone.rotation = normalize(QuaternionMultiply(parentBone.rotation, bone.rotation));
    
    float3 parentUpVec = QuaternionRotateOptimized(parentBone.rotation, upVec);
...
void ProcessBone(int boneId)
{
    ...
    float3 oUpVec = QuaternionRotateOptimized(bone.rotation, upVec);
    ...
    if (speed > 0.001)
    {
        ...
        bone.rotation = normalize(QuaternionMultiply(deltaRotation, bone.rotation));
    }
        ...
        inout TriangleStream<GeoOut> triStream)
{
    ...
    for (int i = 0; i < 5; ++i)
    {
        ...
        float3 rightPos = center + right * halfWidth + float3(0.0f, 1.0f, 0.0f) * bone.length;
        float3 leftPos = center - right * halfWidth + float3(0.0f, 1.0f, 0.0f) * bone.length;
        rightPos = QuaternionRotateOptimized(bone.rotation, rightPos);
        leftPos = QuaternionRotateOptimized(bone.rotation, leftPos);
        ...
        goutLeft.PosW = mul(vLeft, gViewProj);
