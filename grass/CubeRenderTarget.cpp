...
CD3DX12_CPU_DESCRIPTOR_HANDLE CubeRenderTarget::Rtv(int face)
{
	return mCpuRtv[face];
}
...
void CubeRenderTarget::BuildDescriptors()
{
	...
	md3dDevice->CreateShaderResourceView(mCubeMap.Get(), &srvDesc, mCpuSrv);
	...
	for (int i = 0; i < 6; ++i)
	{
		...
		md3dDevice->CreateRenderTargetView(mCubeMap.Get(), &rtvDesc, mCpuRtv[i]);
	}
}
